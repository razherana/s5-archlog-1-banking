...
public class ComptePretDetailController extends HttpServlet {
  ...
  protected void doGet(HttpServletRequest request, HttpServletResponse response)
    ...
    try {
      ...
      PaymentStatusDTO paymentStatus = comptePretService.getPaymentStatus(userAdmin, loanId);
      List<EcheanceDTO> paymentHistory = comptePretService.getPaymentHistory(userAdmin, loanId);
  ...
  protected void doPost(HttpServletRequest request, HttpServletResponse response)
    ...
    try {
      ...
      if ("make_payment".equals(action)) {
        handleMakePayment(request, response, loanId, userAdmin);
      } else {
      ...
      var e = ExceptionUtils.root(ex);

      LOG.severe("Error processing loan action: " + e.getMessage());
    }
  }
